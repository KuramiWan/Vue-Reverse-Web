        ...
        <div class="container">
          ...
          <div class="mainAchieve">
            ...
            <div class="md-layout md-gutter md-alignment-top">
              <div class="md-layout-item md-large-size-30 md-medium-size-33  md-xsmall-size-100 sideBar">
              ...
              <div class="md-layout-item md-large-size-70 md-medium-size-66  md-xsmall-size-100 mainBar">
                <md-field class="md-form-group-outline" slot="inputs">
                  <md-icon>lock_outline</md-icon>
                  <label>查找你的问题</label>
                  <md-input v-model="question"></md-input>
                  <md-button class="md-icon-button md-raised" @click="search">
                    <md-icon>search</md-icon>
                  </md-button>
                </md-field>
                <div v-if="isShow">123</div>
                <timeline v-if="!isShow">
                  <timeline-title class="md-headline" bg-color="#42a5f5">文章总览</timeline-title>

                  <div v-for="timeBlock  in timeBlocks" :key="timeBlock">
                    <timeline-item bg-color="#ec407a">{{timeBlock.dateString}}</timeline-item>
                    <div v-for="(block,index) in timeBlock.archivesListVo " :key="index">
                      <router-link :to="'/detail/' + block.id">
                        <timeline-item icon-size="mudium" bg-color="#42a5f5" @click="goDetail">
                          <div>
                            <div class="md-title fontOverHide">一个标题 {{block.title}}</div>
                            ...
                            <div class="md-body2 fontOverHide"><span class="mate-label">Q:</span>{{block.question}}</div>
                            <div class="md-body fontOverHide"><span class="mate-label">A:</span>{{block.answer}}</div>
                          </div>
                        </timeline-item>
                      </router-link>
                    </div>
                ...
                <pagination v-model="pagination.page" :pageCount="pagination.pageCount" :perPage="pagination.pageSize" @input="input" />
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    search(){
      notification.post('/search',this.question)
      .then(
        response =>{
        ...
        .then((response) => {
          ...
          for (var i in blocks) {
            blocks[i].forEach((block) => {
              block.createTime = new Date(block.createTime).toLocaleDateString()
      ...
      axios
        ...
        .then((response) => {
          this.pagination.pageCount = Math.ceil(
            response.data / this.pagination.pageSize
          )
        })
        .catch((e) => {
          console.log(e)
