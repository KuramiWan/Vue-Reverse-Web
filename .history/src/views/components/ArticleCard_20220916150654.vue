    ...
    <div v-for="(block, index) in searchResult" :key="index">
      <router-link :to="'/detail/' + block.id">
        <div>
          ...
          <div class="md-body2 fontOverHide line">
            <span class="mate-label">Q:</span>{{ block.question }}
          </div>
          <div class="md-body fontOverHide line">
            <span class="mate-label">A:</span>{{ block.answer }}
          </div>
        </div>
      </router-link>
    </div>
