...
export default {
  ...
  methods: {
    kanbanUpdate() {
      ...
      for (statusKey in statusBlocks) {
        console.log(statusBlock);
        for (var key in statusMap) {
          if (statusMap[key] == statusMap[key].status) {
            statusBlocks[statusKey].status = key;
          }
        }

        for (var blockKey in statusBlocks[statusKey].kanbanVoList) {
          block.valueOf.status = statusBlock[block].status;
        }
