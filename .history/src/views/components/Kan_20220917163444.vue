...
export default {
  ...
  methods: {
    kanbanUpdate() {
      ...
      for (var statusBlock in statusBlocks) {
        console.log(statusBlock);
        for (var key in statusMap) {
          if (statusMap[key] == statusBlock.valueOf.status) {
            statusBlock.valueOf.status = key;
          }
        }

        for (var block in statusBlock.valueOf.kanbanVoList) {
          block.valueOf.status = statusBlock.valueOf.status;
        }
        blocks.concat(statusBlock.valueOf.kanbanVoList);
      }
      console.log(blocks);
      notification
