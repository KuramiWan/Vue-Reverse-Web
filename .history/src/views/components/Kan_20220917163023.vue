    ...
    kanbanUpdate() {
      ...
      for (var statusBlock in statusBlocks) {
        ...
        for (var key in statusMap) {
          if (statusMap[key] == statusBlock.status) {
            statusBlock.status = key;
          }
        }

        for (var block in statusBlock.kanbanVoList) {
          block.status = statusBlock.status;
        }
        blocks.concat(statusBlock.kanbanVoList);
      }
      notification
