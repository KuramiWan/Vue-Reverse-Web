...
export default {
  ...
  methods: {
    confirm() {
      if (this.select.edit) {
        notification
          .post("/admin/announcement/update", this.selected, {
            headers: {
      ...
      notification
        .post("/admin/announcement/delete", this.selected, {
          headers: {
            Authorization: localStorage.getItem("Authorization")
          }
        })
    ...
    searchOnTable() {
      this.searched = searchByName(this.announcements, this.search);
      this.input(1);
    },
    input(currentPage) {
    ...
    getForm() {
      notification
        ...
        .then(response => {
          ...
          this.searchOnTable(this.announcements, this.search);
          this.input(1);
        })
        .catch(error => {
          console.log(error);
